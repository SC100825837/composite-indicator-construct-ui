<template>
  <div>
    ...
    <div id="excel-manager">
      ...
      <el-button @click="previewExcelContent" type="primary" size="small" >数据加载</el-button>
      ...
      <!-- <hot-table
        ...
        :data="excelData"
        style="width: 90%;margin-top:35px"
      >
        <hot-column
          :title="item"
          :data="item"
          :key="index"
          v-for="(item, index) in Object.keys(excelData[0] ? excelData[0] : {})"
        >
        </hot-column>
      </hot-table> -->
...
export default {
  ...
  created() {
    //设置初期默认值
    this.previewExcelContent();
  },
  methods: {
    previewExcelContent() {
      $axios.previewExcelContent(18, 1).then((result) => {
